...
Intersector::Intersector(MPI_Comm &comm_, DataManagers3D &dms_, EmbeddedSurfaceData &iod_surface_,
                         ...
                         GlobalMeshInfo &global_mesh_, bool MACGrid_)
           : MACGrid(MACGrid_), comm(comm_), iod_surface(iod_surface_), surface(surface_),
             tree_1(NULL), tree_n(NULL), coordinates(coordinates_), 
             ...
             XForwardMAC(nullptr), XBackwardMAC(nullptr)
{
  ...
  if(MACGrid) {
    XForwardMAC  = new SpaceVariable3D(comm_, &(dms_.ghosted1_3dof));
    XBackwardMAC = new SpaceVariable3D(comm_, &(dms_.ghosted1_3dof));
    XForwardMAC->SetConstantValue(-1, true);
    XBackwardMAC->SetConstantValue(-1, true);
  }
  ...
  if(MACGrid)
    intersectionsMAC.reserve(capacity);
...
Intersector::Destroy()
{
  ...
  XForward.Destroy();
  XBackward.Destroy();
  if(XForwardMAC) XForwardMAC->Destroy();
  if(XBackwardMAC) XBackwardMAC->Destroy();
  Phi.Destroy();
  Color.Destroy();
}
...
Intersector::GetPointerToResults()
{
  ...
  ebds->MACGrid                 = MACGrid;
  ebds->XForwardMAC_ptr         = XForwardMAC;
  ebds->XBackwardMAC_ptr        = XBackwardMAC;
  ebds->intersectionsMAC_ptr    = &intersectionsMAC;
...
void
Intersector::FindIntersectionsMAC() //find intersections only also finds occluded and first layer nodes
{

  // Find nodal candidates, layer = 1
  //
  // TODO I AM HERE. REUSE candidates???
  // 
  FindNodalCandidates(BBmin_1, BBmax_1, tree_1, CandidatesIndex_1, candidates_1);
